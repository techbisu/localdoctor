...
export async function searchHealthcare(params: {
    ...
    const { query, type = "all", lat, lng, radius = 10, city, specialization, page = 1, limit = 12, } = params;
    ...
    if (type === "all" || type === "doctor") {
        ...
        for (const doctor of doctors) {
            ...
            if (lat && lng && doctor.chambers[0]?.latitude) {
                distance = haversineDistance(lat, lng, doctor.chambers[0].latitude, doctor.chambers[0].longitude || 0);
            }
