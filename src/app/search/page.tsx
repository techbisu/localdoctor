  ...
  return (
    <div className='space-y-4'>
      {results.map((result: any) => {
        if (result.type === 'doctor') return <DoctorCard key={result.id} doctor={result} />
        if (result.type === 'pharmacy') return <PharmacyCard key={result.id} pharmacy={result} />
        if (result.type === 'lab') return <LabCard key={result.id} lab={result} />
        return null
      })}
    </div>
  )
