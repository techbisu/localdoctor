        ...
        <div className="flex gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  {doctor.specializations?.slice(0, 3).map((s: any) => (
                    <Badge key={s.specialization.id} variant="secondary" className="text-xs">
              ...
              {doctor.status === "VERIFIED" && (
                <Badge className="bg-teal-700 text-white text-xs shrink-0">Verified</Badge>
              )}
            </div>

            <p className="text-sm text-muted-foreground mt-1">{doctor.qualifications}</p>
            {doctor.experienceYears && (
              <p className="text-sm text-muted-foreground">{doctor.experienceYears} years experience</p>
            )}
            ...
            <div className="flex gap-3 mt-2">
              ...
              {primaryChamber?.phone && (
                <a href={`tel:${primaryChamber.phone}`} className="text-sm text-teal-700 font-medium hover:underline flex items-center gap-1">
